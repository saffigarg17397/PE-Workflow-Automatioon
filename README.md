...
**2. Confidence and human-in-the-loop.** Every field carries `high`/`medium`/`low`. Anything below `high` lands in a review queue showing the value, the source passage, and the reason it was flagged.
...
**Test suite:** 34 tests, no API key required, covering the two-pass join, confidence policy, every rule, thesis scoring, memo rendering, and the web layer.
...
**Effort as the cost lever.** All stages run on the same model; what varies is `output_config.effort` (`config.py: STAGE_EFFORT`). Swapping stages to a cheaper model to save cost trades quality invisibly; effort makes the tradeoff explicit and tunable.
